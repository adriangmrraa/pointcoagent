...
def forward_to_orchestrator(payload: dict, headers: dict):
    response = requests.post(f"{ORCHESTRATOR_URL}/chat", json=payload, headers=headers, timeout=(5, 120))
...
async def process_user_buffer(from_number: str, business_number: str, customer_name: Optional[str], event_id: str, provider_message_id: str):
    ...
    try:
        ...
        raw_res = forward_to_orchestrator(inbound_event, headers)
